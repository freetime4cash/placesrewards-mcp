...
$result=['status'=>'running','started_at'=>date(DATE_ATOM),'view'=>$view,'changes'=>[]];

try {
    if(!is_file($view)) throw new RuntimeException('Member referrals view not found');
    $before=(string)file_get_contents($view);
    $content=$before;
    $backupDir=$agentRoot.'/data/backups/member-referrals';
    @mkdir($backupDir,0755,true);
    $backup=$backupDir.'/index.blade.'.date('Ymd-His').'.php';
    @copy($view,$backup);
    $result['backup']=$backup;

    if(!str_contains($content,'TREASURE_HUNT_REFERRAL_CARD')) {
        ...
        if($start===false || $end===false || $end<=$start) throw new RuntimeException('Could not locate referral card boundaries');

        $replacement=<<<'BLADE'
{{-- TREASURE_HUNT_REFERRAL_CARD --}}
@php
    $referrerCard = $program['settings']->referrerCard ?? null;
    $refereeCard = $program['settings']->refereeCard ?? null;
    $isTreasureHuntReferral = (($referrerCard?->id ?? null) === '95cbd0bf-8bbb-436d-b7c6-a2e1e558db25') || (($refereeCard?->id ?? null) === '95cbd0bf-8bbb-436d-b7c6-a2e1e558db25');
    $referrerPoints = (int) ($program['settings']->referrer_points ?? 0);
    $refereePoints = (int) ($program['settings']->referee_points ?? 0);
@endphp

<div class="relative overflow-hidden rounded-3xl border border-violet-200 dark:border-violet-800/60 bg-gradient-to-br from-violet-50 via-white to-emerald-50 dark:from-violet-950/30 dark:via-secondary-900 dark:to-emerald-950/20 p-7 md:p-8 shadow-lg">
    <div class="space-y-6">
        <div>
            <div class="text-xs font-black tracking-[0.18em] uppercase text-violet-600 dark:text-violet-400 mb-2">
        ...
        <p class="text-base md:text-lg text-secondary-700 dark:text-secondary-300 leading-relaxed">
            ...
            @else
                Share your personal referral link. Places Rewards tracks the invitation, qualifying action, and resulting rewards so both sides can earn together.
            @endif
        </p>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div class="rounded-2xl bg-white/80 dark:bg-secondary-900/80 border border-secondary-200 dark:border-secondary-700 p-4">
                ...
                <div class="font-bold text-secondary-900 dark:text-white">The referral is attributed and rewards are issued</div>
            </div>
        </div>
        ...
        @if($isTreasureHuntReferral)
            <div class="rounded-2xl bg-secondary-900 text-white p-5">
...
BLADE;
        ...
        $result['changes'][]='replaced_loyalty_card_grid_with_referral_content';
    }

    $content=str_replace("{{ trans('common.your_rewards') }}",'Your Referral Program',$content);

    if($content!==$before) {
        if(file_put_contents($view,$content,LOCK_EX)===false) throw new RuntimeException('Could not write referrals view');
    }
