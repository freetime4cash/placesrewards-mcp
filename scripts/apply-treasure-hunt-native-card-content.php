...
$hunterPassportId='95cbd0bf-8bbb-436d-b7c6-a2e1e558db25';
$clueRewardId='29304849-3c10-4a06-8f8f-4bad776b79f9';
$prizeRewardId='13085fc2-2a5d-43ee-92b5-441bc368c55b';

require $appRoot.'/vendor/autoload.php';
$app=require $appRoot.'/bootstrap/app.php';
$app->make(Illuminate\Contracts\Console\Kernel::class)->bootstrap();

function tr(string $value): string {
    return json_encode(['en_US'=>$value,'en'=>$value], JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
}
...
function ensureCardReward(string $cardId,string $rewardId): array {
    if(!Schema::hasTable('card_reward')) return ['table'=>'card_reward','card_id'=>$cardId,'reward_id'=>$rewardId,'status'=>'missing_table'];
    $exists=DB::table('card_reward')->where('card_id',$cardId)->where('reward_id',$rewardId)->exists();
    if(!$exists){
        $values=['card_id'=>$cardId,'reward_id'=>$rewardId];
        if(Schema::hasColumn('card_reward','created_at')) $values['created_at']=now();
        if(Schema::hasColumn('card_reward','updated_at')) $values['updated_at']=now();
        DB::table('card_reward')->insert($values);
        return ['table'=>'card_reward','card_id'=>$cardId,'reward_id'=>$rewardId,'status'=>'inserted'];
    }
    if(Schema::hasColumn('card_reward','updated_at')) DB::table('card_reward')->where('card_id',$cardId)->where('reward_id',$rewardId)->update(['updated_at'=>now()]);
    return ['table'=>'card_reward','card_id'=>$cardId,'reward_id'=>$rewardId,'status'=>'present'];
}

$result=['status'=>'running','updated_at'=>now()->toIso8601String(),'records'=>[],'relationships'=>[]];

$result['records'][]=patchRecord('cards',$hunterPassportId,[
...
$result['records'][]=patchRecord('stamp_cards','a9566430-b6b0-434d-ae3c-f3ba85421c5f',[
    ...
    'valid_from'=>'2026-09-01 00:00:00','valid_until'=>'2030-12-31 23:59:59','is_active'=>1,'is_visible_by_default'=>1,
]);

$result['records'][]=patchRecord('rewards',$clueRewardId,[
    ...
    'active_from'=>'2026-09-01 00:00:00','expiration_date'=>'2030-12-31 23:59:59','max_number_to_redeem'=>100000,'is_active'=>1,
]);
$result['relationships'][]=ensureCardReward($hunterPassportId,$clueRewardId);
...
$result['records'][]=patchRecord('rewards',$prizeRewardId,[
...
$result['relationships'][]=ensureCardReward($hunterPassportId,$prizeRewardId);
...
$result['records'][]=patchRecord('vouchers','14788f52-438e-4293-bd6b-c82b8e448983',[
    ...
    'type'=>'fixed_amount','value'=>500,'currency'=>'USD','min_purchase_amount'=>2500,'is_active'=>1,'is_public'=>1,'is_visible_by_default'=>1,'is_single_use'=>1,
]);
$result['records'][]=patchRecord('vouchers','358b51e6-189c-4b17-95b2-eb2794d5b763',['is_active'=>0,'is_public'=>0,'is_visible_by_default'=>0]);
...
$result['status']='completed';$result['completed_at']=now()->toIso8601String();@mkdir(dirname($out),0755,true);file_put_contents($out,json_encode($result,JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES),LOCK_EX);echo json_encode(['status'=>'completed','record_count'=>count($result['records']),'relationships'=>$result['relationships']]),"\n";
