...
$result=['status'=>$overall,'workflow_url'=>'https://app.placesrewards.com/demo/treasure-hunt','links'=>$links,'locale_demo_bypass'=>$localeBypassStatus,'native_card_content'=>$nativeContentStatus,'native_card_visuals'=>$nativeVisualStatus,'runtime_inspection'=>$runtimeInspection,'destination_inspection'=>$destinationInspection,'scratch_instance_inspection'=>$scratchInstanceInspection,'middleware_inspection'=>$middlewareInspection,'actual_link_verification'=>$actualLinkVerification,'deployed_at'=>now()->toIso8601String()];
@mkdir(dirname($resultPath),0755,true);file_put_contents($resultPath,json_encode($result,JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES),LOCK_EX);echo json_encode(['status'=>$overall,'module_count'=>12,'scratch_instance_inspection'=>$scratchInstanceInspection,'actual_link_verification'=>$actualLinkVerification,'workflow_url'=>$result['workflow_url']]),"\n";exit($overall==='completed'?0:1);
