...
const VALID_AGENTS = new Set(["command","build","test","demo"]);
...
export class AgentOrchestrator {
  ...
  async enqueue({ agent, objective, input, priority = 50, autonomyLevel = 2, maxAttempts = 3 }) {
    ...
    const job = { id: randomUUID(), agent, objective, status: "queued", priority, autonomyLevel, attempts: 0, maxAttempts, createdAt: now, updatedAt: now, ...(input !== undefined ? { input } : {}) };
  ...
  tasks(data) {
    ...
    return data.tasks.filter(t => t && typeof t === "object" && VALID_AGENTS.has(t.agent) && typeof t.objective === "string");
  }
  ...
  async runNext() {
    ...
    try {
      const result = await agent.run({ jobId: running.id, objective: running.objective, input: running.input ?? {} });
      const decisions = this.actions(result.data).map(action => ({ action, decision: evaluateAction(action.risk, job.autonomyLevel) }));
  ...
  async approveJob(id, approvedAutonomyLevel = 3) {
    ...
    return this.store.update(id, { status: "queued", autonomyLevel: approvedAutonomyLevel, currentStep: "Approved and returned to queue" });
  }
