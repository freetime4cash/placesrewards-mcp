...
export class CommandAgent {
  ...
  async run(context) {
    return {
      ok: true,
      summary: `Command analyzed: ${context.objective}`,
      data: {
        tasks: [
          { agent: "build", objective: "Inspect the live Laravel Places Rewards application and Agent API, identify concrete capability gaps, and prepare the next safe development action.", priority: 90, autonomyLevel: 2, input: context.input },
          { agent: "test", objective: "Run non-destructive Laravel diagnostics and verify the live Places Rewards Agent API.", priority: 80, autonomyLevel: 2 },
          { agent: "demo", objective: "Maintain the demo-write safety gate and prepare demo-only work for approval.", priority: 70, autonomyLevel: 2 }
        ]
      }
    };
  }
...
export class BuildAgent {
  ...
  async run(context) {
    ...
    if (input.operation === "apply_patch") {
      return {
        ok: true,
        summary: "Build Agent requests approval to apply a prepared patch to production.",
        data: {
          proposalId: input.proposalId,
          actions: [{ name: "apply_patch_to_production", risk: "protected_write", details: { proposalId: input.proposalId, execute: false } }]
        }
      };
    }

    if (input.operation === "execute_approved_patch") {
      ...
      return {
        ok: true,
        summary: "Approved patch was applied and passed validation.",
        data: { applied, tests, actions: [{ name: "production_patch_applied_and_verified", risk: "read", details: { proposalId: input.proposalId, backupDir: applied.backupDir } }] }
      };
    }
